...
async function fetchMoviesFromEndpoint(endpoint, apiKey) {
    const apiUrl = `https://api.themoviedb.org/3/${endpoint}?api_key=${apiKey}&language=en-US&page=1`;
...
async function fetchAndDisplayMoviesFromTMDB(sectionId, endpoint, apiKey) {
    const section = document.getElementById(sectionId);
    const movieList = section.querySelector('.movie-list');
    const movies = await fetchMoviesFromEndpoint(endpoint, apiKey);
...
async function displayMovies() {
    ...
    await fetchAndDisplayMoviesFromTMDB('new', 'movie/now_playing', apiKey);
    await fetchAndDisplayMoviesFromTMDB('next', 'movie/popular', apiKey);
    await fetchAndDisplayMoviesFromTMDB('today', 'movie/top_rated', apiKey);
}
