...
async function fetchMoviesFromEndpoint(endpoint, apiKey, query) {
    const apiUrl = `https://api.themoviedb.org/3/search/${endpoint}?query=${query}&api_key=${apiKey}`;
...
async function displayMovies() {
    ...
    await fetchAndDisplayMoviesFromTMDB('movie', 'movie', apiKey, `${data.title}`);
    await fetchAndDisplayMoviesFromTMDB('tv', 'tv', apiKey, `${data.title}`);
}
